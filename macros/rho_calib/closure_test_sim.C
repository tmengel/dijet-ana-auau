...
#ifndef _CLOSURE_TEST_SIM_C_
#define _CLOSURE_TEST_SIM_C_
...
int closure_test_sim(
    ...
    const std::string & outplot = "",
    const std::string & outfile = ""
)
{
    const std::string treefile = Form( "%s/%s/rho_calib_%s.root", tree_dir.c_str(), label.c_str(), label.c_str() );
    const std::string calibfile = Form( "%s/rho_calib_%s.root", calib_dir.c_str(), label.c_str() );
    const std::string plotpath = outplot.empty() ? Form( "/sphenix/user/tmengel/dijet-ana-auau/macros/rho_calib/closure_test_%s.pdf", label.c_str() ) : outplot;
    const std::string filepath = outfile.empty() ? Form( "/sphenix/user/tmengel/dijet-ana-auau/macros/rho_calib/closure_test_%s.root", label.c_str() ) : outfile;
    ...
    // -- plots + summary --
    auto * fout = new TFile( filepath.c_str(), "RECREATE" );
    auto * c = new TCanvas( "c", "closure test", 1800, 700 );
    c->Divide( 3, 1 );
    for ( int ic = 0; ic < kNCalo; ++ic )
    {
        c->cd( ic + 1 );
        ...
        pRaw[ic]->GetYaxis()->SetTitleOffset( 1.6 );
        pRaw[ic]->GetYaxis()->SetLabelSize( 0.035 );
    ...
    std::cout << "Wrote " << filepath << " and " << plotpath << std::endl;
    return 0;
}
