...
#ifndef _MAKE_RHO_CALIB_C_
#define _MAKE_RHO_CALIB_C_
...
// Reads the per-event tower statistics trees produced by read_tree.C (one
// per run, output/<run>/rho_calib_<run>.root) and derives the per-(layer,
...
int make_rho_calib(
    ...
    // are dropped from the calibration. Defaults were chosen from the
    // zvrtx/mbdQ distribution of run 54912 (|zvrtx|<60 minbias sample):
    // zvtx roughly follows the beam profile (median ~0, 10-90% ~[-14,+18]),
    // mbdQ is steeply falling (10-90% ~[10,1400]) as expected for a minbias
    // trigger, so the mbdQ edges are spaced to give comparable statistics
    // per bin rather than equal width.
...
)
{
    ...
    const std::string infile = Form( "/sphenix/user/tmengel/dijet-ana-auau/macros/rho_calib/output/%d/rho_calib_%d.root", run_number, run_number );
    const std::string outfile = Form( "%s/rho_calib_%d.root", outdir.c_str(), run_number );
    const std::string qa_outfile = Form( "%s/rho_calib_%d_qa.root", outdir.c_str(), run_number );
