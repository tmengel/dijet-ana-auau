...
#ifndef _FINAL_CROSSCHECK_FULLTOWER_C_
#define _FINAL_CROSSCHECK_FULLTOWER_C_
...
// Same bias/width cross-check as final_crosscheck.C, but computed at the
// FULL per-tower granularity (each of the up to 64 phi-towers in a ring
// treated as its own data point) directly from the anatree files, instead
// of on read_tree.C's phi-averaged mean_E[ieta] (which dilutes true
// tower-level fluctuation by ~sqrt(64)). Also adds a 3rd background
// prediction: the SUB1/iterative method's own per-(layer,ieta) UE estimate
// (sub2_towerbkgd_ue, written by AnaTreev1 from TowerBackgroundv1::get_UE()
// -- this is what SubtractTowers.cc literally subtracts from each tower,
// new_energy = raw_energy - UE, already in absolute tower-energy units, no
// cosh(eta) factor needed) -- so uncal/cal rho vs sub1 can be compared on
// equal footing.
//
// Scope: anatree files are ~2.9GB each (61 files, 162GB total for run
// 54590) with full tower_E[3][24][64] arrays, so this reads only the first
// nfiles_max of them (default 2, ~55k events, ~250M tower-samples) -- ample
// statistics for tight per-(layer,ieta) mean/RMS, not a full-run pass.
int final_crosscheck_fulltower(
    const int run = 54590,
...
)
{
    ...
    // -- build chain over the first nfiles_max anatree files for this run --
    ...
    for ( int seg = 0; seg < 999 && nfiles_added < nfiles_max; ++seg )
    {
        ...
        if ( gSystem->AccessPathName( fn.c_str() ) ) continue; // returns 0 if exists
        ch->Add( fn.c_str() );
        ++nfiles_added;
        std::cout << "Added " << fn << std::endl;
    }
    ...
    enable( "sub2_towerbkgd_ue", sub2_ue );

    const Long64_t nentries = ch->GetEntries();
    std::cout << "Total entries in chain: " << nentries << std::endl;

    // ===================== accumulate, per (layer,ieta), at FULL per-tower granularity =====================
    static const int kNMethod = 3; // 0=uncal(rho) 1=cal(rho, static w) 2=sub1(TowerBackgroundv1 UE)
    const char * method_names[kNMethod] = { "uncal (rho)", "cal (rho, static w)", "sub1 (iterative UE)" };
    ...
    std::cout << n_events_used << " / " << nentries << " events used (minbias, |zvrtx|<" << max_abs_zvrtx
              << ", in-range z/mbdQ bin)." << std::endl;
    std::cout << "Total per-tower samples (cemc, ieta=0): n=" << accRaw[0][0].n << std::endl;

    // ===================== plots: bias & width vs ieta, 3 methods, full-tower stats =====================
    ...
    auto * cBias = new TCanvas( "c_bias_ft", "fractional bias (full tower)", 1800, 700 );
    cBias->Divide( 3, 1 );
    auto * cWidth = new TCanvas( "c_width_ft", "fractional width (full tower)", 1800, 700 );
    cWidth->Divide( 3, 1 );

    int colors[kNMethod] = { kAzure + 2, kRed + 1, (int) kSpring + 4 };
    // method 0 (uncal) is excluded from the plots -- its much larger bias/width dominates the
    // y-range and hides the (more interesting) differences between the correction methods
    // themselves. Still computed & reported in the console summary below for reference.
    ...
    for ( int ic = 0; ic < kNCalo; ++ic )
    {
        ...
        if ( ic == 0 )
        {
            auto * leg = new TLegend( 0.13, 0.72, 0.55, 0.89 );
        ...
        std::cout << "\n[" << calo_names[ic] << "] eta-averaged |fractional bias| and fractional width by method (full-tower stats):" << std::endl;
        for ( int m = 0; m < kNMethod; ++m )
        {
            double sb = 0, sw = 0;
            for ( int ieta = 0; ieta < NETA; ++ieta )
    ...
    cBias->SaveAs( Form( "%s/crosscheck_fulltower_bias.png", outdir.c_str() ) );
    cBias->SaveAs( Form( "%s/crosscheck_fulltower_bias.pdf", outdir.c_str() ) );
    cWidth->SaveAs( Form( "%s/crosscheck_fulltower_width.png", outdir.c_str() ) );
    cWidth->SaveAs( Form( "%s/crosscheck_fulltower_width.pdf", outdir.c_str() ) );

    std::cout << "\nWrote crosscheck_fulltower_{bias,width}.{png,pdf} in " << outdir << std::endl;
    return 0;
}
